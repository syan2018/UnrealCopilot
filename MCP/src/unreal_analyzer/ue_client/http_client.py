...
class UEPluginClient:
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            raise UEPluginError(
                ...
                f"Please ensure the UE Editor is running with UnrealProjectAnalyzer plugin enabled. "
                f"Original error: {e}"
            ) from e
    ...
    async def get_with_async(
        ...
        *,
        ...
        if isinstance(response, dict) and response.get("mode") == "async" and response.get("job_id"):
        ...
        *,
        ...
        while True:
            ...
            if (time.monotonic() - start_t) > timeout_s:
                raise UEPluginError(f"Async job timeout after {timeout_s}s (id={job_id}, status={state})")
        ...
        while offset < total_chars:
            part = await self.get("/analysis/job/result", {
                "id": job_id,
                "offset": offset,
                "limit": chunk_size,
            })
